...
# class GPMin:
#     def __init__(self, model, likelihood, xNormalizer, lr=0.1):
#         self.model = model
#         self.likelihood = likelihood
#         self.xNormalizer = xNormalizer
#         self.lr = lr
...
#         h = Variable(
#             self.xNormalizer.transform(rand_torch(-30, 30, 2, 200)),
#             requires_grad=True,
#         )
#         # t = Variable(torch.tensor([[0.0, 0.0]]), requires_grad=True)
#         optimizer = torch.optim.Adam([h], lr=self.lr)

#         for i in range(training_steps):
#             optimizer.zero_grad()
#             output = self.model(h)
#             loss = torch.sum(output.mean)
#             loss.backward()
#             optimizer.step()

#             with torch.no_grad():
#                 h = self.xNormalizer.itransform(h)
#                 h[:] = h.clamp(-30, +30)
#                 h = self.xNormalizer.transform(h)

#         loss = self.model(h).mean

#         minIdx = torch.argmin(loss)

#         return h[minIdx].detach().numpy()
#         # return rand(-15, 15, 2)
