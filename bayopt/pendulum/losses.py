...
class PendulumErrorWithConstraintRandomInit(Loss):
    ...
    def evaluate(self, k):
        ...
        for i in range(self.N):
            ...
            norm += (2 - np.linalg.norm(X_star-X_bo)/np.sqrt(self.c.n_simulation))/self.N
            c1 += (np.pi*np.pi/ 8- np.max((X_star[:, 0] - X_bo[:, 0])**2))/self.N
            # c2 += (np.pi*np.pi - np.max((X_star[:, 1] - X_bo[:, 1])**2))/self.N
            c2 += (10 - np.max(np.abs(X_bo[:, 1])))/self.N
...
class PendulumErrorWithConstraint(Loss):
    ...
    def evaluate(self, k):
        ...
        norm = (-np.linalg.norm(X_star-X_bo)/np.sqrt(self.c.n_simulation))
        c1 = (10- np.max((X_star[:, 0] - X_bo[:, 0])**2))
        c2 = (10 - np.max(np.abs(X_bo[:, 1])))
